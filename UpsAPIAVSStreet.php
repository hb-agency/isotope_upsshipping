 ...
 * @filesource
 */
class UpsAPIAVSStreet extends UpsAPI {
	...
	public function buildRequest($customer_context = null) {
		...
		$request_element = $this->buildRequest_RequestElement(
			$addressconfirm_element, 'XAV', '3', $customer_context);
					
		$address = $addressconfirm_element->appendChild(new DOMElement('AddressKeyFormat'));
	...
	protected function buildRequest_Address(&$dom_element) {
		...
		$strConsignee = $this->destination['company'] ? $this->destination['company'] : $this->destination['name'];
		
		$dom_element->appendChild(new DOMElement('ConsigneeName',
			$strConsignee));
			
		$dom_element->appendChild(new DOMElement('AddressLine',
		...
		$dom_element->appendChild(new DOMElement('PoliticalDivision2',
			$this->destination['city']));
		$dom_element->appendChild(new DOMElement('PoliticalDivision1',
			$this->destination['state']));
		$dom_element->appendChild(new DOMElement('PostcodePrimaryLow',
